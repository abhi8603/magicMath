function set_timer(block, time_end,section, function_result) {
    ...
    let timer,
        ...
        _day = _hour * 24,
        set_storage = (section) => {
            if (!localStorage.getItem('timer_start_' + section)) {
                let now = new Date();

                localStorage.setItem('timer_start_' + section, Math.round(now.getTime() / 1000));
            }
        },
        update_settings = (section) => {
            start = end = localStorage.getItem('timer_start_' + section);
