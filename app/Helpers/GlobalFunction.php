...
function session_year($onlyActive = false)
{
    if ($onlyActive) {
        $onlyActive = 1;
    }
    $session_year = DB::table('session_settings')->select('session')->when($onlyActive, function ($query) use ($onlyActive) {
            return $query->where('status', '=', $onlyActive);
        })->groupBy('session')->orderBy('id','desc')->get();
    return $session_year;
}
...
function paginateArray($data, $perPage = 15)
{
    ...
    $total = count($data);
      if($page > $total){
        $minus=$page-$total;
        $results = array_slice($data, ($page - ($minus+1)) * $perPage, $perPage);
    }else{
